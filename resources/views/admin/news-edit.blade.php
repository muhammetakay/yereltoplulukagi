                        ...
                        <div class="col-lg-6">
                            <form action="{{ route('admin.news.edit', $news->id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="content" id="content" rows="5">{{ old('content') ?: $news->content }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label for="category">Kategori</label>
                                    <select name="category" id="category" class="form-control">
                                        @foreach (getAllCategories() as $item)
                                            <option value="{{ $item->id }}" @selected((old('category') ?: $news->category_id) == $item->id)>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
